...
public class UserInterceptor implements Interceptor {
	...
	public void intercept(Invocation ai) {
		...
		String site = "http://" + request.getServerName()+"/admin";
...
//			}
			...
			if ((user == null || user.getUserid() <= 0) && (tmpPath.indexOf("/") > -1 || "/admin/home".equals(tmpPath))) {
			...
			if (JFlyFoxUtils.isBack(tmpPath)) {
				if (user == null || user.getUserid() <= 0) {
					controller.redirect("/admin/trans");
					return;
				}
			}
			ai.invoke();
